...
// ---------- BOT TRIGGER ENDPOINTS ----------
// These endpoints are called by GitHub Actions workflows.
// They run from inside Render with Render's fixed outbound IP —
// so API-Football's IP allowlist only needs to be set once.

app.get("/api/bot/my-ip", requireAdmin, async (req, res) => {
  try {
    const r = await fetch("https://api.ipify.org?format=json");
    const data = await r.json();
    res.json({ outbound_ip: data.ip, message: "Whitelist this IP on API-Football dashboard" });
  } catch (err) {
    res.status(500).json({ error: "Could not determine outbound IP", detail: err.message });
  }
});

app.post("/api/bot/sync-fixtures", requireAdmin, async (req, res) => {
  try {
    const { syncFixtures } = require("./bot/tasks");
    const days = parseInt((req.body && req.body.days) || 5);
    const result = await syncFixtures(days);
    res.json({ success: true, ...result });
  } catch (err) {
    console.error("[BOT fixtures] Fatal:", err.message);
    res.status(500).json({ success: false, error: err.message });
  }
});

app.post("/api/bot/sync-form", requireAdmin, async (req, res) => {
  try {
    const { syncForm } = require("./bot/tasks");
    const days = parseInt((req.body && req.body.days) || 2);
    const result = await syncForm(days);
    res.json({ success: true, ...result });
  } catch (err) {
    console.error("[BOT form] Fatal:", err.message);
    res.status(500).json({ success: false, error: err.message });
  }
});

app.post("/api/bot/sync-livescores", requireAdmin, async (req, res) => {
  try {
    const { syncLivescores } = require("./bot/tasks");
    const result = await syncLivescores();
    res.json({ success: true, ...result });
  } catch (err) {
    console.error("[BOT livescores] Fatal:", err.message);
    res.status(500).json({ success: false, error: err.message });
  }
});

app.post("/api/bot/enrich-matches", requireAdmin, async (req, res) => {
  try {
    const { enrichMatches } = require("./bot/tasks");
    const days = parseInt((req.body && req.body.days) || 5);
    const result = await enrichMatches(days);
    res.json({ success: true, ...result });
  } catch (err) {
    console.error("[BOT enrich] Fatal:", err.message);
    res.status(500).json({ success: false, error: err.message });
  }
});

app.post("/api/bot/reset-db", requireAdmin, async (req, res) => {
  try {
    const { resetDB } = require("./bot/tasks");
